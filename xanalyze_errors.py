...
def parse_args():
    ...
    return results_file, top_n, examples_n, show_all


def main():
    results_file, top_n, examples_n, show_all = parse_args()
    summary_file = summary_file_for(results_file)
    original_stdout = sys.stdout
    summary_fh = open(summary_file, "w", encoding="utf-8")
    sys.stdout = Tee(original_stdout, summary_fh)
    try:
        run_analysis(results_file, top_n, examples_n, show_all)
        print()
        print(f"{results_file} summarized in {summary_file}")
    finally:
...
def run_analysis(results_file, top_n, examples_n, show_all):
    print(results_file)
    errors = collect_errors(results_file)
