...
program test_statistics_mod
use statistics_mod
implicit none
real(kind=dp) :: x(6), y(6), prices(4), xmat(3,2)
real(kind=dp) :: thresh(2), perc(3), wgt(6)
integer       :: ivec(6), bins(5)
logical       :: mask(6), tf(6)

x      = [1.0_dp, 2.0_dp, 4.0_dp, 8.0_dp, 16.0_dp, 32.0_dp]
y      = [2.0_dp, 1.0_dp, 3.0_dp, 5.0_dp, 8.0_dp, 13.0_dp]
prices = [100.0_dp, 105.0_dp, 102.0_dp, 110.0_dp]
xmat   = reshape([1.0_dp, 2.0_dp, 3.0_dp, 4.0_dp, 5.0_dp, 6.0_dp], [3, 2])
thresh = [3.0_dp, 10.0_dp]
perc   = [0.25_dp, 0.50_dp, 0.75_dp]
wgt    = [1.0_dp, 1.0_dp, 2.0_dp, 2.0_dp, 3.0_dp, 3.0_dp]
ivec   = [1, 1, 2, 3, 5, 8]
bins   = [1, 2, 1, 3, 2]
mask   = [.true., .false., .true., .true., .false., .true.]
tf     = [.false., .true., .true., .false., .true., .false.]

print *, "mean", mean(x)
print *, "mean_int", mean_int(ivec)
print *, "geo_mean", geo_mean(x)
print *, "range", range(x)
print *, "range_mean", range_mean(x)
print *, "rms", rms(x)
print *, "vol_rms", vol_rms(x)
print *, "sd", sd(x)
print *, "sd_fixed_mean", sd_fixed_mean(x, mean(x))
print *, "sd_wgt", sd_wgt(x, wgt)
print *, "sd_weighted", sd_weighted(x, wgt, .false.)
print *, "semidev", semidev(x, 10.0_dp)
print *, "semisharpe", semisharpe(x, 0.0_dp)
print *, "skew", skew(x)
print *, "kurtosis", kurtosis(x)
print *, "prob_positive", prob_positive(x - 5.0_dp)
print *, "concentration", concentration(x)
print *, "mean_abs_diff", mean_abs_diff(x)
print *, "mean_abs_sum", mean_abs_sum(x)
print *, "roughness", roughness(x)
print *, "num_wgt", num_wgt(x)
print *, "count_signs", count_signs(x - 5.0_dp)
print *, "num_changes_int", num_changes(ivec)
print *, "num_changes_real", num_changes(x)
print *, "num_changes_tf", num_changes(tf)
print *, "num_sign_changes", num_sign_changes(x - 5.0_dp)
print *, "frequency_changes", frequency_changes(x)
print *, "bin_scalar", bin(5.0_dp, [3.0_dp], [10.0_dp])
print *, "bin_freq", bin_freq(thresh, x)
print *, "bin_prob", bin_prob(thresh, x)
print *, "bin_counts", bin_counts(bins, 3)
print *, "num_bin_changes", num_bin_changes(x, thresh)
print *, "mean_by_col", mean_by_col(xmat)
print *, "mean_by_row", mean_by_row(xmat)
print *, "sd_by_col", sd_by_col(xmat)
print *, "sharpe_by_col", sharpe_by_col(xmat)
print *, "mean_2d", mean_2d(xmat)
print *, "mean_mask", mean_mask(x, mask)
print *, "first", first(x)
print *, "last", last(x)
print *, "minval_def", minval_def(x, -1.0_dp)
print *, "maxval_def", maxval_def(x, -1.0_dp)
print *, "correl", correl(x, y)
print *, "slope", slope(x, y)
print *, "cumul_sum", cumul_sum(y)
print *, "diff", diff(x, 1)
print *, "diff_nonoverlap", diff_nonoverlap(x, 2)
print *, "diff_rms", diff_rms(x, 1)
print *, "running_mean", running_mean(x)
print *, "rollmean", rollmean(x, 3)
print *, "rollmean_12", rollmean_12(x, 1, 3)
print *, "sums", sums(x, 3)
print *, "moving_averages", moving_averages(x, 3)
print *, "sums_nonoverlap", sums_nonoverlap(x, 2)
print *, "norm_abs", norm_abs(x)
print *, "cumul_ret", cumul_ret([1.0_dp, -2.0_dp, 3.0_dp])
print *, "cagr", cagr([1.0_dp, 2.0_dp, -1.0_dp])
print *, "compound_ret", compound_ret([1.0_dp, 2.0_dp, -1.0_dp], "sum", 100.0_dp)
print *, "acf", acf(x, 2)
print *, "stdz", stdz(x)
print *, "aic_penalty", aic_penalty(3, 20)
print *, "inf_crit_mvn", inf_crit_mvn("aic", 3, 20, 1.5_dp)
end program test_statistics_mod
